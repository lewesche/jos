...
file_block_walk(struct File *f, uint32_t filebno, uint32_t **ppdiskbno, bool alloc)
{
	...
	if(filebno < NDIRECT) {
		*ppdiskbno = &(f->f_direct[filebno]); // might be 0, thats OK
		...
		if(f->f_indirect==0) {
			if(alloc) {
				...
				memset(diskaddr(new_block), 0, PGSIZE);
			} else {
				return -E_NOT_FOUND;
			}
		}
		// At this point there is a indirect block
		uint32_t *ind = diskaddr(f->f_indirect);
...
file_get_block(struct File *f, uint32_t filebno, char **blk)
{
	...
	//If no block is mapped here alloc one
	if(*ppdiskbno == 0) {
		int new_block = alloc_block();
		if(new_block < 0) {
			return -E_NO_DISK;
