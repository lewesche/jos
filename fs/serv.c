...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	ssize_t n_read = file_read(o->o_file, ret->ret_buf, req->req_n, o->o_fd->fd_offset);
	if(n_read < 0) {return n_read;}
	
	o->o_fd->fd_offset += n_read;
	return n_read;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	int n_written = file_write(o->o_file, req->req_buf, req->req_n, o->o_fd->fd_offset);
	if(n_written < 0) {return n_written;}

	o->o_fd->fd_offset += n_written;
	return n_written;
}
